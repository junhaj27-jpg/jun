...
class ModelRuntime:
    """기본 모델과 LoRA 어댑터를 한 번만 올리고 TASK별로 어댑터만 전환한다."""
    ...
    def start(self) -> "ModelRuntime":
        ...
        with self._load_lock:
            ...
            self.model_context_limit = int(getattr(text_config, "max_position_embeddings", MODEL_CONTEXT_LIMIT_FALLBACK))
            kwargs: dict[str, Any] = {"token": self.hf_token, "device_map": {"": 0}, "attn_implementation": ATTN_IMPLEMENTATION, "low_cpu_mem_usage": True}
            if USE_4BIT:
            ...
            self.model = PeftModel.from_pretrained(base_model, STAGE1_ADAPTER_REPO, adapter_name="stage1", token=self.hf_token, is_trainable=False, low_cpu_mem_usage=True, autocast_adapter_dtype=False)
            self.model.eval()
    ...
    def _ensure_stage3(self) -> None:
        ...
        if "stage3" not in self.model.peft_config:
            self.model.load_adapter(STAGE3_ADAPTER_REPO, adapter_name="stage3", token=self.hf_token, is_trainable=False, low_cpu_mem_usage=True, autocast_adapter_dtype=False)
    ...
    def _tokenize(self, messages: list[dict]) -> dict[str, torch.Tensor]:
        ...
        inputs = self.tokenizer.apply_chat_template(normalized, tokenize=True, add_generation_prompt=True, return_dict=True, return_tensors="pt")
    ...
    def _max_new_tokens(self, task_type: str, prompt_tokens: int, explicit_cap: int | None = None) -> int:
        policy = GENERATION_POLICY[task_type]
        estimated = int(explicit_cap if explicit_cap is not None else prompt_tokens * float(policy["multiplier"]))
        estimated = max(estimated, int(policy["minimum"]))
        estimated = min(estimated, int(policy["maximum"]))
        actual = min(estimated, self.model_context_limit - prompt_tokens - GENERATION_SAFETY_MARGIN)
        if actual < 1:
            raise RuntimeError(f"{task_type}: context limit 초과. prompt={prompt_tokens:,}")
        return int(actual)

    @torch.inference_mode()
    def run_task(self, task_type: str, user_obj: dict, *, raw_log_path: Path | None = None) -> tuple[dict, str]:
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    if status["free_gb"] < MINIMUM_FREE_GPU_GB:
                        raise RuntimeError(f"{task_type}: GPU 여유 메모리 부족. free={status['free_gb']:.2f}GB")
                    inputs = {k: v.to(self.model.device) for k, v in cpu_inputs.items() if isinstance(v, torch.Tensor)}
                    input_length = int(inputs["input_ids"].shape[-1])
                    started = time.perf_counter()
                    generated = self.model.generate(**inputs, max_new_tokens=max_new_tokens, do_sample=False, use_cache=True, eos_token_id=self.tokenizer.eos_token_id, pad_token_id=self.tokenizer.pad_token_id)
                    elapsed = time.perf_counter() - started
                    generated_ids = generated[0, input_length:]
                    generated_count = int(generated_ids.shape[-1])
                    raw_text = self.tokenizer.decode(generated_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False).strip()
                    hit_token_limit = generated_count >= max_new_tokens
                    record = {"attempt": attempt, "adapter_name": adapter_name, "prompt_tokens": prompt_tokens, "max_new_tokens": max_new_tokens, "generated_tokens": generated_count, "hit_token_limit": hit_token_limit, "elapsed_seconds": elapsed, "raw_text": raw_text}
                    if raw_log_path:
                        dump_json(raw_log_path.with_name(f"{raw_log_path.stem}_attempt{attempt}.json"), {"task_type": task_type, "user": user_obj, **record})
                    if hit_token_limit:
                        raise RuntimeError(f"{task_type}: 생성 토큰 한도 도달")
                    ...
                    if parse_mode.startswith("json_repair") and not raw_text.rstrip().endswith("}"):
                        raise RuntimeError(f"{task_type}: 불완전 json_repair 결과 거부")
                    obj = validate_task_output(task_type, obj)
                    record["parse_mode"] = parse_mode
                    record["prediction"] = obj
                    attempts.append(record)
                    if raw_log_path:
                        dump_json(raw_log_path, {"status": "SUCCESS", "task_type": task_type, "adapter_name": adapter_name, "user": user_obj, "attempts": attempts, "prediction": obj})
                    return obj, raw_text
                    ...
                    if attempt < max_attempts:
                        ...
                        explicit_cap = min(int(previous * float(policy["retry_growth"])), int(policy["maximum"]))
                        current_messages = base_messages + [{"role": "user", "content": f"직전 출력은 JSON 검증에 실패했다. 설명 없이 학습된 출력 스키마의 완전한 JSON 객체 하나만 다시 출력하라. 오류: {str(exc)[:1500]}"}]
                        continue
                    if raw_log_path:
                        dump_json(raw_log_path.with_name(raw_log_path.stem + "_FAILED.json"), {"status": "FAILED", "task_type": task_type, "adapter_name": adapter_name, "user": user_obj, "attempts": attempts})
                    raise
                finally:
